...
def draw(canvas):
    global time, started, rock_group, missile_group, explosion_group, my_ship, lives, score
...
def rock_spawner():
    global rock_group, started, asteroid_info
    ...
    if started:
        if len(rock_group) < 12:
            if dist (rock_pos, my_ship.get_position()) > asteroid_info.get_radius() + my_ship.get_radius() +20:
...
def process_sprite_group(group, canvas):
    ...
    for sprite in group:
        if sprite.update():
            removeSprite.add(sprite)
        else:
            sprite.draw(canvas)
    if len (removeSprite) > 0:
        group.difference_update(removeSprite)
...
def group_collide(group, other_object):
    ...
    for sprite in group:
        if sprite.collide(other_object):
            ...
            collisions += 1
            removeObject.add (sprite)

    if len (removeObject) > 0:
        group.difference_update (removeObject)
...
def group_group_collide(group_1, group_2):
    ...
    removeObject = set ([])
    collisions = 0
    for object in group_1:
        if group_collide(group_2, object) > 0:
            removeObject.add(object)
            collisions += 1
    if len(removeObject) > 0:
        group_1.difference_update(removeObject)
    return collisions
